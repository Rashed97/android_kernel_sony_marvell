...
static int snd_card_bt_capture_close(struct snd_pcm_substream *substream)
{
	...
	struct snd_bt_chip *chip =
	    (struct snd_bt_chip *)substream->private_data;
...
static ssize_t snd_card_bt_pcm_receive(unsigned char *data, unsigned int len)
{
				...
				size_t count, loff_t *ppos)
{
	ssize_t len = 0;
	mutex_lock(&lock);
	if(reference_count && (snd_bt_card != NULL)) {
		len = snd_card_bt_pcm_receive(buf, count);
	}
	mutex_unlock(&lock);
