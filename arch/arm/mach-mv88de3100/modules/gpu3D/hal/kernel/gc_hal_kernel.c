...
//###modified for marvell-bg2
        /* set 0x14 to 0xF7BC0008[7..0] to avoid CPU & CPU IO bridge conflict */
        {
            gctUINT32 val;

            gckOS_ReadRegisterEx(kernel->hardware->os,
                                 kernel->hardware->core,
                                 0x8,
                                 &val);
            val &= ~0xFF;
            val |= 0x14;
            gckOS_WriteRegisterEx(kernel->hardware->os,
                                  kernel->hardware->core,
                                  0x8,
                                  val);
        }
...
//####modified for marvell-bg2
        ...
        if (node->VidMem.memory->object.type == gcvOBJ_VIDMEM){
            ...
            Interface->u.LockVideoMemory.memory= gcvNULL;     //set to NULL means remap
        }
        else{
#if gcdSECURE_USER
...
//####end for marvell-bg2
    ...
    case gcvHAL_CACHE:
        ...
        else if (Interface->u.Cache.node->VidMem.memory == gcvNULL)
        {
            status = gcvSTATUS_OK;
            break;
        }
