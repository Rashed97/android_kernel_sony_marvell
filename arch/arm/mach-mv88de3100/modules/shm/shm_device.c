...
int shm_device_get_meminfo(shm_device_t * device, pMV_SHM_MemInfo_t pInfo)
{
...

	{
		...
		for (node = device->m_engine->m_root.m_next_free; node->m_size != 0; node = node->m_next_free) {
			if (node->m_size > max_free_size) {
				max_free_size = node->m_size;
			}
		}

		pInfo->m_max_freeblock = max_free_size;
	}
