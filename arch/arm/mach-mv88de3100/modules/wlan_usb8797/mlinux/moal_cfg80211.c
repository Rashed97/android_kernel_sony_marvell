...
woal_cfg80211_deinit_p2p(moal_private * priv)
{
    ...
    if (priv->phandle->remain_on_channel) {
        ...
        if (priv->phandle->cookie) {
            cfg80211_remain_on_channel_expired(remain_priv->netdev,
                                               priv->phandle->cookie,
                                               &priv->phandle->chan,
                               ...
                               const struct cfg80211_bitrate_mask *mask)
{
    ...
    if (priv->phandle->card_type == CARD_TYPE_USB8797) {
...
#define PACKET_ADDR4_POS		(2 + 2 + 6 + 6 + 6 + 2)
    ...
    case MLAN_STATUS_PENDING:
        ...
        /* delay 30ms to guarantee the packet has been already tx'ed becuase if
