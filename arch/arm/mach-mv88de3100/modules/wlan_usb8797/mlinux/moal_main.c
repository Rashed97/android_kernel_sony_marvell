...
/** Driver version */
char driver_version[] =
    "USB8797-%s-M" KERN_VERSION "14" MLAN_RELEASE_VERSION
    "-GPL" "-(" "FP" FPNUM ")"
#ifdef	DEBUG_LEVEL2
    "-dbg"
#endif
...
woal_update_drv_tbl(moal_handle * handle, int drv_mode_local)
{
    ...
    if (fw_name) {
        ...
        if (handle->card_type == CARD_TYPE_USB8782)
...
woal_init_sw(moal_handle * handle)
{
    ...
    if (handle->card_type == CARD_TYPE_USB8782)
        memcpy(driver_version, CARD_USB8782, strlen(CARD_USB8782));
    else if (handle->card_type == CARD_TYPE_USB8797)
        memcpy(driver_version, CARD_USB8797, strlen(CARD_USB8797));
...
woal_init_priv(moal_private * priv, t_u8 wait_option)
{
    ...
    if (priv->bss_type == MLAN_BSS_TYPE_WIFIDIRECT) {
        if (priv->bss_virtual) {
            if (priv->pa_netdev) {
                memcpy(priv->current_addr, priv->pa_netdev->dev_addr, ETH_ALEN);
...
woal_add_card(void *card)
{
    ...
    /* Update card type */
    handle->card_type = ((struct usb_card_rec *) card)->card_type;
