...
static irqreturn_t pe_devices_vip_isr(int irq, void *dev_id)
{
	...
	if (bTST(instat, avioDhubSemMap_vip_hdmirx_intr)) {
		...
		GA_REG_BYTE_READ(SOC_VPP_BASE + (HDMIRX_INTR_STATUS_2 << 2), &stat2);
	        GA_REG_BYTE_READ(SOC_VPP_BASE + (HDMIRX_INTR_EN_2 << 2), &en2);
		en2 = en2 & stat2;

		GA_REG_BYTE_READ(SOC_VPP_BASE + (HDMIRX_INTR_STATUS_3 << 2), &stat3);
		GA_REG_BYTE_READ(SOC_VPP_BASE + (HDMIRX_INTR_EN_3 << 2), &en3);
		en3 = en3 & stat3;
		if (  (en2 & (HDMI_RX_INT_SYNC_DET | HDMI_RX_INT_VRES_CHG | HDMI_RX_INT_HRES_CHG)) || (en3 & (HDMI_RX_INT_5V_PWR | HDMI_RX_INT_CLOCK_CHANGE)) )
		{
			// calibrate trigger
...
#if CONFIG_HRX_IOCTL_MSG
	init_MUTEX_LOCKED(&hrx_sem);
	err = PEMsgQ_Init(&hPEHRXMsgQ, HDMIRX_ISR_MSGQ_SIZE);
	if (unlikely(err != S_OK)) {
		pe_trace("PEHRXMsgQ_Init: falied, err:%8x\n", err);
		return err;
	}
#endif
#endif
...
static int pe_device_exit(unsigned int cpu_id, void *pHandle)
{
	...
	if (unlikely(err != S_OK)) {
		pe_trace("hrx MsgQ Destroy: falied, err:%8x\n",err);
		return err;
	}
#endif
#endif
				     ...
				     unsigned long arg)
{
	...
	case HDMIRX_IOCTL_GET_MSG:
	{
		...
		if (unlikely(rc != S_OK)) {
			pe_trace("HRX read message queue failed\n");
			return -EFAULT;
		}
