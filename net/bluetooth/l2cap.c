...
/* GES:IAN:20111011 Due to issues with setting up the Makefile using this    */
/* ugly method to include C code into the l2cap file. This will not be       */
/* required in newer kernels as l2cap.c has been split into multiple files.  */
/* So going with this hack for now.                                          */
#include "l2cap_ucd.h"
#endif
...
#ifdef CONFIG_BT_UCD
static u32 l2cap_feat_mask = (L2CAP_FEAT_FIXED_CHAN | L2CAP_FEAT_UCD_RX);
#else
static u32 l2cap_feat_mask = L2CAP_FEAT_FIXED_CHAN;
#endif

static u8 l2cap_fixed_chan[8] = { 0x06, };
...
#if 1
	/*03/21/2012: Reverting MRVL security block fix to switch to the fix provided by Sony*/

	/*Commented out for BT remote will failed to reconnect due to security block reason issue:
	BT remote reconnect refused due to security block reason.
	As from air log, encryption event is not reached host yet, but the L2CAP packet reached first.*/
...
static inline int l2cap_information_req(struct l2cap_conn *conn, struct l2cap_cmd_hdr *cmd, u8 *data)
{
	...
	if (type == L2CAP_IT_CL_MTU) {
		u8 buf[6];
		struct l2cap_info_rsp *rsp = (struct l2cap_info_rsp *) buf;
		rsp->type   = cpu_to_le16(L2CAP_IT_CL_MTU);
		rsp->result = cpu_to_le16(L2CAP_IR_SUCCESS);
		put_unaligned_le16(L2CAP_DEFAULT_MTU, rsp->data);
		l2cap_send_cmd(conn, cmd->ident,
			L2CAP_INFO_RSP, sizeof(buf), buf);
	} else if (type == L2CAP_IT_FEAT_MASK) {
		u8 buf[8];
...
#ifdef CONFIG_BT_UCD
	if (l2cap_ucd_receive(conn, psm, skb)) {
		sk = NULL;
		goto drop;
	}
#endif
...
#ifdef CONFIG_BT_UCD
	if (l2cap_ucd_init()) {
		BT_ERR("Failed to init L2CAP UCD subsystem");
	}
#endif
