...
int hci_dev_cmd(unsigned int cmd, void __user *arg)
{
	...
	case HCISETLINKMODE:
		hdev->link_mode = ((__u16) dr.dev_opt | HCI_LM_MASTER) &
...
int hci_register_dev(struct hci_dev *hdev)
{
	...
	hdev->link_mode = (HCI_LM_ACCEPT | HCI_LM_MASTER);
...
static inline void hci_acldata_packet(struct hci_dev *hdev, struct sk_buff *skb)
{
  ...
  bool isRC = true;
	...
	BT_DBG("conn: dev_class[2, 1, 0]=0x%.2x%.2x%.2x", conn->dev_class[2], conn->dev_class[1], conn->dev_class[0]);

	if (conn) {
		register struct hci_proto *hp;

    for (i = 0; i < 6; i++) {
      if (bdaddrRc.b[i] != conn->dst.b[i]) {
        isRC = false;
        break;
      }
    }

    if (!isRC && (0x05 != (0x1f & conn->dev_class[1]))) {
			hci_conn_enter_active_mode(conn);
		}
