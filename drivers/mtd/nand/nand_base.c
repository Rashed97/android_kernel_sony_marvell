			   ...
			   const uint8_t *buf, int page, int cached, int raw)
{
	...
	if (chip->mtd_flush_need) {
		struct mtd_info *mtd_dp = (struct mtd_info *)chip->mtd_flush_need(mtd,
							NAND_CMD_PAGEPROG, 0, page);
		if (mtd_dp != NULL) {
			chip->cmdfunc(mtd_dp, NAND_CMD_NAND_CACHE_FLUSH, 0, 0);
		}
	}
...
static void nand_sync(struct mtd_info *mtd)
{
	...
	chip->cmdfunc(mtd, NAND_CMD_NAND_CACHE_FLUSH, 0, 0);
