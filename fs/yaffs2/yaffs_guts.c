...
void yaffs_HandleUncorrectableChunkError(yaffs_Device *dev, yaffs_BlockInfo *bi)
{
	if (!bi->gcPrioritise) {
		bi->gcPrioritise = 1;
		dev->hasPendingPrioritisedGCs = 1;
		bi->needsRetiring = 1; /* Uncorrectable erro, so retire this */
		yaffs_BackgroundGarbageCollect(dev, 1);
		T(YAFFS_TRACE_ALWAYS, (TSTR("yaffs: Block struck out" TENDSTR)));
	}
}
					...
					int useReserve)
{
	...
	/* Set up new tags */
	memset(&newTags, 0, sizeof(newTags));
...
static void yaffs_FlushFilesChunkCache(yaffs_Object *obj)
{
	...
	int i;
	yaffs_ChunkCache *cache;
	int chunkWritten = 0, totalWritten = 0;
	int nCaches = obj->myDev->param.nShortOpCaches;
	...
	if (nCaches > 0) {
		...
		if (totalWritten) {
			yaffs_FlushDevInternalCache(dev);
		}
...
void yaffs_FlushDevInternalCache(yaffs_Device *dev)
{
	T(YAFFS_TRACE_SYNC, (TSTR("Flush NAND cache" TENDSTR)));
	if (dev->param.syncNAND)
		dev->param.syncNAND(dev);
}
...
int yaffs_GutsInitialise(yaffs_Device *dev)
{
	...
	if (!init_failed && dev->param.isYaffs2 &&
